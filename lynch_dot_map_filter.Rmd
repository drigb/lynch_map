...
setwd('/Users/david/Dropbox/Lynchings_full/Socius Revisions/')
dot_data <- read.csv("/Users/david/Dropbox/Lynchings_full/datasets/make_dot_map_with_this_02.csv", colClasses='character', stringsAsFactors=F)
...
lynchings$year <- as.numeric(lynchings$year)
...
```{r}
filter_slider("year", "Year", lynch_share, column=~`year`, step=5)
filter_checkbox("race_count", "Race of Victim", lynch_share, ~`race_count`, inline = TRUE)
filter_checkbox("gender", "Gender of Victim", lynch_share, ~`gender`, inline = TRUE)
filter_checkbox("state", "State", lynch_share, ~`state`, inline = TRUE)
```
